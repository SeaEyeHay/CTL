...
extern void make_ptr_deq (void* ret, size_t items, size_t length);
...
extern void deq_get_ptr (void* restrict ret, void* restrict deq, size_t item, size_t max, size_t offset, size_t i);

extern void deq_set_ptr (void* restrict val, void* restrict deq, size_t item, size_t max, size_t offset, size_t i);

extern void deq_add_ptr (void* restrict deq, size_t item, size_t* restrict length, size_t max, 
                         size_t* restrict offset, size_t i);

extern void deq_rem_ptr (void* restrict deq, size_t item, size_t* restrict length, size_t max, 
                         size_t* restrict offset, size_t i);


extern void deq_push_ptr (void* restrict deq, void* restrict val, size_t item, size_t* restrict length, 
                          size_t max, size_t offset);

extern void deq_push_front_ptr (void* restrict deq, void* restrict val, size_t item,
                                size_t* restrict length, size_t max, size_t* restrict offset);


extern void deq_pop_ptr (void* restrict ret, void* restrict deq, size_t item, size_t* restrict length, 
                         size_t max, size_t offset);

extern void deq_pop_front_ptr (void* restrict ret, void* restrict deq, size_t item, 
...
CTL_INLINE struct CTL_STRUCT DEF_CONSTRUCTOR(CTL_TYPE_NAME, deq) (size_t iniSize) fn ( 
    ...
    make_ptr_deq (&newVec, sizeof(CTL_TYPE_ID), iniSize);
...
CTL_INLINE CTL_TYPE_ID DEF_METHODE(deq, get, CTL_TYPE_NAME) (struct CTL_STRUCT* deq, size_t i) fn (
    CTL_TYPE_ID ret;
    deq_get_ptr (&ret, deq->store, sizeof(CTL_TYPE_ID), deq->max, deq->off, i);
...
CTL_INLINE CTL_TYPE_ID DEF_METHODE(deq, set, CTL_TYPE_NAME) (struct CTL_STRUCT* deq, size_t i, CTL_TYPE_ID x) fn ( 
    ...
    deq_set_ptr (&x, deq->store, sizeof(CTL_TYPE_ID), deq->max, deq->off, i);
...
CTL_INLINE void DEF_METHODE(deq, add, CTL_TYPE_NAME) (struct CTL_STRUCT* deq, size_t i, CTL_TYPE_ID x) fn (
    ...
    deq_add_ptr (deq->store, sizeof(CTL_TYPE_ID), &deq->len, deq->max, &deq->off, i);
    deq_set_ptr (&x, deq->store, sizeof(CTL_TYPE_ID), deq->max, deq->off, i);
)

CTL_INLINE CTL_TYPE_ID DEF_METHODE(deq, rem, CTL_TYPE_NAME) (struct CTL_STRUCT* deq, size_t i) fn (
    CTL_TYPE_ID ret = DEF_METHODE(deq, get, CTL_TYPE_NAME) (deq, i);
    deq_rem_ptr (deq->store, sizeof(CTL_TYPE_ID), &deq->len, deq->max, &deq->off, i);
...
CTL_INLINE void DEF_METHODE(deq, push, CTL_TYPE_NAME) (struct CTL_STRUCT* deq, CTL_TYPE_ID x) fn (
    if (deq->len == deq->max) grow_deq ((void**)&deq->store, sizeof(CTL_TYPE_ID), &deq->max, &deq->off);
    deq_push_ptr (deq->store, &x, sizeof(CTL_TYPE_ID), &deq->len, deq->max, deq->off);
)

CTL_INLINE void DEF_METHODE(deq, push_front, CTL_TYPE_NAME) (struct CTL_STRUCT* deq, CTL_TYPE_ID x) fn (
    if (deq->len == deq->max) grow_deq ((void**)&deq->store, sizeof(CTL_TYPE_ID), &deq->max, &deq->off);
    deq_push_front_ptr (deq->store, &x, sizeof(CTL_TYPE_ID), &deq->len, deq->max, &deq->off);
)


CTL_INLINE CTL_TYPE_ID DEF_METHODE(deq, pop, CTL_TYPE_NAME) (struct CTL_STRUCT* deq) fn (
    CTL_TYPE_ID ret;
    deq_pop_ptr (&ret, deq->store, sizeof(CTL_TYPE_ID), &deq->len, deq->max, deq->off);
...
CTL_INLINE CTL_TYPE_ID DEF_METHODE(deq, pop_front, CTL_TYPE_NAME) (struct CTL_STRUCT* deq) fn (
    CTL_TYPE_ID ret;
    deq_pop_front_ptr (&ret, deq->store, sizeof(CTL_TYPE_ID), &deq->len, deq->max, &deq->off);
